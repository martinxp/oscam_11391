...
static int32_t irdeto_card_init(struct s_reader *reader, ATR *newatr)
{
	...
	if((reader->caid == 0x0653) || (reader->caid == 0x0650) || (reader->caid == 0x0666) || (reader->caid == 0x0624 && csystem_data->acs57 == 1))    // acs 6.08 and ice 0D96/0624
	{
		camkey = 4;
		sc_Acs57CamKey[2] = 0;
	}
...
// There are also other readers suffering from simmilar issue for those cards.
	...
	case 4:
	{
		...
		if((reader->caid == 0x0653) || (reader->caid == 0x0650) || (reader->caid == 0x0666) || (reader->caid == 0x0624 && csystem_data->acs57 == 1))
		{
...
int32_t irdeto_do_ecm(struct s_reader *reader, const ECM_REQUEST *er, struct s_ecm_answer *ea)
{
	...
	if(csystem_data->acs57 == 1)
	{
		...
		if((reader->caid == 0x0653) || (reader->caid == 0x0650) || (reader->caid == 0x0666) || (reader->caid == 0x0624 && csystem_data->acs57 == 1))    //crc for orf, cslink, skylink
		{
			sc_Acs57Ecm[2] = 0;
			crc ^= 0x01;
			crc ^= 0x05;
	...
	if(ok)
	{
		...
		if(l <= ADDRLEN)
		{
			if(csystem_data->acs57 == 1)
			{
				...
				if(ep->type == GLOBAL && ((reader->caid == 0x0624 && csystem_data->acs57 == 1) || reader->caid == 0x0653 || reader->caid == 0x0650 || reader->caid == 0x0666)) { dataLen += 2; }
				int32_t crc = 63;
