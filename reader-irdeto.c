...
unsigned char Camkey[] = {0xB1,0xB2,0xB3,0xB4,0xB5,0xB6,0xB7,0xB8};
unsigned char  
CamKeyData[] = { 0xAB, 0x72, 0x2A, 0x8C, 0xAA,
		0x9F, 0x58, 0xDE, 0x0D, 0x1C, 0xBB, 0x6F, 0xF3,
		0x29, 0xC9, 0x61, 0xED, 0x9D, 0xD7, 0x59, 0xB1,
		0x03, 0x08, 0x2D, 0x1B, 0xAE, 0x64, 0xC2, 0x7A,
		0x86, 0x1D, 0x42, 0xE7, 0xF6, 0x31, 0x89, 0x1C,
		0x26, 0x60, 0xEF, 0xA5, 0x74, 0x84, 0xF8, 0x0B,
		0xBB, 0x4E, 0x0E, 0x82, 0x49, 0xA8, 0x36, 0x8E,
		0x31, 0xBD, 0x3F, 0xFC, 0x36, 0x9F, 0xBE, 0x95,
		0x16, 0x05, 0x28
			};
...
static time_t chid_date(struct s_reader *reader, uint32_t date, char *buf, int32_t l)
{
	...
	CHID_BASE_DATE table[] = { 
		...
		{0x0653, 0x0006, "CZE", 946598400L},    // 30.12.1999, 16:00    //frs
		{0x0653, 0x0006, "SVK", 946598400L},    // 30.12.1999, 16:00    //frs
...
static int32_t irdeto_card_init(struct s_reader *reader, ATR *newatr)
{
	...
	if((reader->caid == 0x0648) || (reader->caid == 0x0650) || (reader->caid == 0x0653) || (reader->caid == 0x0624 && csystem_data->acs57 == 1))    // acs 6.08 and ice 0D96/0624
	{
		camkey = 4;
		sc_Acs57CamKey[2] = 0;
	}
...
// There are also other readers suffering from simmilar issue for those cards.
	...
	case 4:
	{
		...
		if((reader->caid == 0x0648) || (reader->caid == 0x0650) || (reader->caid == 0x0653) || (reader->caid == 0x0624 && csystem_data->acs57 == 1))
		{
...
int32_t irdeto_do_ecm(struct s_reader *reader, const ECM_REQUEST *er, struct s_ecm_answer *ea)
{
	...
	if(csystem_data->acs57 == 1)
	{
		...
		if((reader->caid == 0x0648) || (reader->caid == 0x0650) || (reader->caid == 0x0653) || (reader->caid == 0x0624 && csystem_data->acs57 == 1))    //crc for orf, cslink, skylink
		{
			sc_Acs57Ecm[2] = 0;
			crc ^= 0x01;
			crc ^= 0x05;
	...
	if(ok)
	{
		...
		if(l <= ADDRLEN)
		{
			if(csystem_data->acs57 == 1)
			{
				...
				if(ep->type == GLOBAL && ((reader->caid == 0x0624 && csystem_data->acs57 == 1) || reader->caid == 0x0648 || reader->caid == 0x0650 || reader->caid == 0x0653)) { dataLen += 2; }
				int32_t crc = 63;
				...
				{
					if(ep->type == GLOBAL && ((reader->caid == 0x0624 && csystem_data->acs57 == 1) || reader->caid == 0x0648 || reader->caid == 0x0650 || reader->caid == 0x0653))
					{
